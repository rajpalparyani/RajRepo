## Perl Code to implement EC2 API Calls - Telenav Modified
#Modified by Rajpal Paryani 04/04/2013
package main;
...
sub API_RunInstance_Telenav {
    ...
    #Telenav Specific Parameters
    my $subnet 		 = getRequiredParam("subnet", $opts);
    my $private_ip   = getOptionalParam("privateIPAddress", $opts);
        ...
        if ($response->isSetDescribeInstancesResult()) {
            ...
            foreach (@$resList) {
                ...
                foreach (@$instanceList) {
                    ...
                    my $publicIP  = $instance->getPublicDnsName();
                    my $privateIP = $instance->getPrivateDnsName();
                    my $private_ip_address = $instance->getPrivateIpAddress();

mesg(1, "Private ip of $id instance is $privateIP\n");
mesg(1, "AMI of $id instance is $image\n");
mesg(1, "Root Device Type of $id instance is $rtype\n");
mesg(1, "Public ip of $id instance is $publicIP\n");
mesg(1, "Private IP Address of $id instance is $private_ip_address\n");
                    ...
                    if ("$propResult" ne "") {
                        ...
                        $opts->{pdb}->setProp("$propResult/Instance-$id/PrivateIPAddress",  "$private_ip_address");
