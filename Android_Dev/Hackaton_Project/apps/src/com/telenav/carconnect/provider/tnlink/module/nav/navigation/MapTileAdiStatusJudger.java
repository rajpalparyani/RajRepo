...
class MapTileAdiStatusJudger
{
 // judging result indicates current GPS location is not on any roads for sure
    ...
    private int latMin = Integer.MAX_VALUE;
    private int latMax = Integer.MIN_VALUE;
    private int lonMin = Integer.MAX_VALUE;
    private int lonMax = Integer.MIN_VALUE;
    ...
    public void setMapTiles(Vector mapTiles)
    {
        ...
        if (mapTiles != null)
        {
            ...
            {
                MapTile tile = (MapTile)mapTiles.elementAt(i);
                
                int gxCenter = MapUtil.calcGlobalCenterX(tile.getId(), tile.getZoom());
                int gyCenter = MapUtil.calcGlobalCenterY(tile.getId(), tile.getZoom());
                int latCenter = GlobalCoordinateUtil.globalToEarthLat(gyCenter);
                int lonCenter = GlobalCoordinateUtil.globalToEarthLon(gxCenter);
                
                if (latCenter < latMin) latMin = latCenter;
                if (latCenter > latMax) latMax = latCenter;
                if (lonCenter < lonMin) lonMin = lonCenter;
                if (lonCenter > lonMax) lonMax = lonCenter;
                
            }//end for 
        }//end if 
        
        int latCenter = (latMin + latMax)/2;
        int lonCenter = (lonMin + lonMax)/2;
        
        latMin += (latMin - latCenter) / 2;
        latMax += (latMax - latCenter) / 2;
        lonMin += (lonMin - lonCenter) / 2;
        lonMax += (lonMax - lonCenter) / 2;
    }
    
    /**
     *  judge current GPS is on road , in ADI status or there is no enough 
     *  map tile data to make judgment 
     */
    public int isOnRoad(TnNavLocation gpsFix, int[] adiThresholds, boolean ignoreHeading)
    {
        if (tiles == null || tiles.size() == 0 || gpsFix == null) return STATUS_NO_DATA;
        
        int lat = gpsFix.getLatitude();
        int lon = gpsFix.getLongitude();
        
        // check if location is inside the bounding box of map tiles first
        boolean isInsideMapTiles = false;
        for (int i = 0; i < adiThresholds.length; i++)
        {
            if (lat + adiThresholds[i] > latMin &&  lat - adiThresholds[i] < latMax &&
                lon + adiThresholds[i] > lonMin &&  lon - adiThresholds[i] < lonMax)
            {
                isInsideMapTiles = true;
                break;
            }
        }
        
        // current location is not covered by cached map tiles, treat it as deviation
        // it means with client will not support on route ADI with this implementation
        if (!isInsideMapTiles) return STATUS_NO_DATA;
        ...
        for (int i = 0; i < tiles.size(); i++)
        {
            ...
            for (int j = 0; j < edgeSize; j++)
            {
                ...
                if (edge.getRoadType() < DataUtil.RAMP
                        || edge.getRoadType() > DataUtil.HIGHWAY)
                {
            ...
            if (lineClip(point1, point2, leftTop, rightBotton))
            {
                ...
                if (0 == nP12P2 || (nCrossProduct / nP12P2) <= (nThreshold * nThreshold))
                {
                    ...
                    else 
                    {
                        ...
                        if (headingDiff < headingThresh 
                                || (180 - headingDiff) < headingThresh)
                        {
                            return true;
                        }
                    }
