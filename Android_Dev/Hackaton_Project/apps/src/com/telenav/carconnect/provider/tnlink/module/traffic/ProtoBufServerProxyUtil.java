...
import com.telenav.data.serverproxy.impl.IOneBoxSearchProxy.MultiMatchResult;
...
import com.telenav.datatypes.route.DecimatedRoute;
...
import com.telenav.j2me.framework.protocol.ProtoDecimateRoute;
...
import com.telenav.j2me.framework.protocol.ProtoLanPatten;
import com.telenav.j2me.framework.protocol.ProtoLaneInfo;
import com.telenav.j2me.framework.protocol.ProtoMapTile;
...
import com.telenav.j2me.framework.protocol.ProtoRoute;
import com.telenav.j2me.framework.protocol.ProtoRouteSegment;
...
class ProtoBufServerProxyUtil
{
    ...
    public static Address convertAddress(ProtoStopPoiWrapper protoStopPoiWrapper)
    {
        ...
        else
        {
            if(protoStopPoiWrapper.hasStop())
            {
                ...
                if(address.getLabel() == null || address.getLabel().trim().length() <= 0 )
                {
                    String strView = address.getStop().getLabel() ;
    ...
    public static MultiMatchResult covertProtoSuggestionItem(ProtoSuggestionItem item)
    {
        if (item == null)
            return null;
        
        MultiMatchResult result = new MultiMatchResult();
        result.displayLabel = item.getDisplayLabel();
        result.searchKeyWord = item.getQueryStr();
        return result;
    }
    ...
    public static Poi convertPoi(ProtoPoiDetail protoPoiDetail)
    {
        ...
        if (protoBizPoi != null)
        {
            ...
            BizPoi bizPoi = convertBizPoi(protoBizPoi);
    ...
    private static BizPoi convertBizPoi(ProtoBizPoi protoBizPoi)
    {
        ...
        bizPoi.setCategoryId(protoBizPoi.getCategoryId());
//        bizPoi.setCategoryName(protoBizPoi.getCategoryName());//TODO no protoBizPoi.getCategoryName
...
//        bizPoi.setDistanceWithUnit(protoBizPoi.getDistanceWithUnit());//TODO no protoBizPoi.getDistanceWithUnit
        bizPoi.setPhoneNumber(protoBizPoi.getPhoneNumber());
