 ...
 *@date 2010-11-10
 */
class RimMatrix4f extends TnMatrix4f
{
    protected RimMatrix4f()
    {
        super();
    }
    ...
    {
        Matrix4f resultM = new Matrix4f(result, resultOffset);
        Matrix4f lhsM = new Matrix4f(lhs, lhsOffset);
        Matrix4f rhsM = new Matrix4f(rhs, rhsOffset);
        Matrix4f.multiply(lhsM, rhsM, resultM);
        float[] tmpResult = resultM.getArray();
        System.arraycopy(tmpResult, 0, result, 0, result.length);
    }

    // public void multiplyMV(float[] resultVec, int resultVecOffset, float[] lhsMat, int lhsMatOffset, float[] rhsVec,
    // int rhsVecOffset)
    // {
