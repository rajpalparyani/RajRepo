...
public class ActionHandler extends AbstractHandler
{
    ...
    private static final String PACKAGE_NAME = "scout_us";
    ...
    private void launchMaitai(HashMap<String, String> map, String maitaiAction)
    {
        ...
        String action = context.getPackageName() + ".TeleNav.WIDGET";
        Intent intent = new Intent();
        intent.setAction(action);
        intent.addCategory("com.telenav.intent.category.TELENAV");
        intent.putExtra("REQUEST_URI", request.toString());
    ...
    private String getWidgetType()
    {
        ...
        if (widgetParameter != null)
        {
            ...
            else if (R.layout.searchwidget_mini_search == layoutId || R.layout.searchwidget_mini_search_land == layoutId
                    || R.layout.searchwidget_mini_cat == layoutId || R.layout.searchwidget_mini_cat_land  == layoutId)
            {
                widgetType=VALUE_BY_SEARCH_WIDGET;               
            }
            else if(R.layout.searchwidget_half==layoutId||R.layout.searchwidget_half_land==layoutId)
            {
        ...
        if(list != null && list.size() > 0)
        {
            ...
            for(ResolveInfo info : list)
            {
                ...
                if(activityInfo != null && activityInfo.packageName != null 
                        && activityInfo.packageName.indexOf(PACKAGE_NAME) > -1)
                {
                    return true;
                }
            }
